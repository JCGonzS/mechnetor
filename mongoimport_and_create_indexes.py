...
# for sps in ["Xla", "Ath", "Cel", "Dme", "Dre", "Sce"]:#"Hsa", Mmu"]:
    ...
    data_file = (sps_dir+"BIOGRID-ORGANISM-"+sps+"-"+BIOGRID_VERSION+
                ".tab2.txt.gz")
    if (os.path.isfile(data_file)
    and ("import" in sys.argv[1:] or biogrid_data.count()==0)):
        os.system("zcat "+data_file+" | mongoimport -d "+sps+
                  " -c biogrid --type tsv --headerline --drop")
    biogrid_data.drop_indexes()
    biogrid_data.create_index(
                        [("Official Symbol Interactor A", pymongo.ASCENDING),
                        ("Official Symbol Interactor B", pymongo.ASCENDING)])
